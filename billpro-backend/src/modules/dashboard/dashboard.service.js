...
const toObjId = (id) => mongoose.Types.ObjectId(id);
...
// ─── KPI Cards ────────────────────────────────────────────────────────────────
const getKPICards = async (companyId) => {
  ...
  const [
    ...
    SalesInvoice.aggregate([
      { $match: { companyId: companyObjId, invoiceDate: { $gte: currMonth.start, $lte: currMonth.end }, isVoid: false, invoiceType: INVOICE_TYPES.TAX_INVOICE } },
      { $group: { _id: null, total: { $sum: '$grandTotal' }, profit: { $sum: { $subtract: ['$grandTotal', '$totalTaxableValue'] } } } },
    ]),
    // Previous month sales
    SalesInvoice.aggregate([
      { $match: { companyId: companyObjId, invoiceDate: { $gte: prevMonth.start, $lte: prevMonth.end }, isVoid: false, invoiceType: INVOICE_TYPES.TAX_INVOICE } },
      { $group: { _id: null, total: { $sum: '$grandTotal' } } },
    ]),
    // Current month purchase
    PurchaseBill.aggregate([
      { $match: { companyId: companyObjId, billDate: { $gte: currMonth.start, $lte: currMonth.end }, isVoid: false } },
      { $group: { _id: null, total: { $sum: '$grandTotal' } } },
    ]),
    // Previous month purchase
    PurchaseBill.aggregate([
      { $match: { companyId: companyObjId, billDate: { $gte: prevMonth.start, $lte: prevMonth.end }, isVoid: false } },
      { $group: { _id: null, total: { $sum: '$grandTotal' } } },
    ]),
    ...
    SalesInvoice.countDocuments({ companyId: companyObjId, invoiceDate: { $gte: currMonth.start, $lte: currMonth.end }, isVoid: false }),
    // Bill count this month
    PurchaseBill.countDocuments({ companyId: companyObjId, billDate: { $gte: currMonth.start, $lte: currMonth.end }, isVoid: false }),
    // Inventory value
    Inventory.aggregate([
      { $match: { companyId: companyObjId } },
      { $group: { _id: null, total: { $sum: '$totalValue' } } },
    ]),
    // Active customers this FY
    SalesInvoice.distinct('customerId', { companyId: companyObjId, isVoid: false }),
    // Active vendors this FY
    PurchaseBill.distinct('vendorId', { companyId: companyObjId, isVoid: false }),
    // Overdue invoices
    SalesInvoice.aggregate([
      { $match: { companyId: companyObjId, balanceDue: { $gt: 0 }, dueDate: { $lt: new Date() }, isVoid: false } },
...
// ─── Monthly Sales Trend ──────────────────────────────────────────────────────
const getMonthlySalesTrend = async (companyId, fyStartMonth = 4) => {
...
const getSalesVsPurchase = async (companyId, fyStartMonth = 4) => {
  const { start, end } = getFYRange(fyStartMonth);
...
const getTopCustomers = async (companyId, fyStartMonth = 4, limit = 10) => {
  const { start, end } = getFYRange(fyStartMonth);
  return SalesInvoice.aggregate([
    {
      $match: {
