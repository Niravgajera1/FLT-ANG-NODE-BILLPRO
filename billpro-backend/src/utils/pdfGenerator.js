...
/**
 * Compile purchase bill data into a stunning, print-ready HTML and convert it to a PDF buffer using Puppeteer
 * @param {Object} bill - The purchase bill document
 * @param {Object} company - The company profile document
 * @param {Object} vendor - The vendor profile document
 * @returns {Promise<Buffer>} - Resolves to a PDF buffer
 */
const generatePurchaseBillPDF = async (bill, company, vendor) => {
  // Address formatter helper
  const formatAddress = (addr) => {
    if (!addr) return '';
    return [
  ...
  const companyAddressHtml = formatAddress(company.registeredAddress);
  const vendorAddressHtml = formatAddress(bill.vendorAddress || vendor.registeredAddress);

  // Line items rows generator
  const lineItemsHtml = bill.lineItems.map((item, index) => {
    return `
      <tr>
        <td style="text-align: center; border-bottom: 1px solid #e2e8f0; padding: 10px 8px; vertical-align: top;">${index + 1}</td>
        <td style="border-bottom: 1px solid #e2e8f0; padding: 10px 8px; vertical-align: top;">
...
<body>
  ...
  <div class="header-container">
    ...
    <div class="bill-title-section">
      ...
      <table class="bill-meta-table" style="float: right;">
        <tr>
          <td><strong>Bill No:</strong></td>
          <td style="color: #0f172a; font-weight: bold; font-size: 12px;">${bill.billNumber}</td>
        </tr>
        ${bill.vendorBillNumber ? `
        <tr>
          <td><strong>Vendor Bill No:</strong></td>
          <td>${bill.vendorBillNumber}</td>
        </tr>` : ''}
        <tr>
          <td><strong>Bill Date:</strong></td>
          <td>${new Date(bill.billDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
        </tr>
        <tr>
          <td><strong>Vendor Bill Date:</strong></td>
          <td>${new Date(bill.vendorBillDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
        </tr>
        <tr>
          <td><strong>Due Date:</strong></td>
          <td>${bill.dueDate ? new Date(bill.dueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}</td>
        </tr>
        ${bill.poReference ? `
        <tr>
  ...
  <div class="addresses-grid">
    <div class="address-box">
      <div class="address-title">Vendor (Billing From)</div>
      <div class="address-content">
        <strong>${bill.vendorName || vendor.name}</strong><br/>
        ${vendorAddressHtml}<br/>
        ${bill.vendorGSTIN ? `<strong>GSTIN:</strong> ${bill.vendorGSTIN}` : vendor.gstin ? `<strong>GSTIN:</strong> ${vendor.gstin}` : ''}
      </div>
    </div>
    <div class="address-box">
      <div class="address-title">Ship To (Billing To)</div>
      <div class="address-content">
        <strong>${company.legalName}</strong><br/>
        ${companyAddressHtml}<br/>
  ...
  <div class="totals-section">
    <div>
      ${bill.narration ? `
        <div style="margin-top: 15px; max-width: 400px; font-size: 11px;">
          ...
          <span style="color: #64748b;">${bill.narration}</span>
        </div>` : ''}
      ${bill.paymentTerms ? `
        <div style="margin-top: 10px; max-width: 400px; font-size: 11px;">
          <strong>Payment Terms:</strong> <span style="color: #64748b;">${bill.paymentTerms}</span>
        </div>` : ''}
    </div>
    ...
    <div>
      <table class="totals-table">
        ...
        <tr>
          <td>Discount:</td>
          <td style="text-align: right; color: #dc2626;">-₹${bill.totalDiscount.toFixed(2)}</td>
        </tr>` : ''}
        ${bill.totalCGST > 0 ? `
        <tr>
          <td>CGST:</td>
        ...
        <tr>
          <td>IGST:</td>
          <td style="text-align: right;">₹${bill.totalIGST.toFixed(2)}</td>
        </tr>` : ''}
        ${bill.roundOff !== 0 ? `
        <tr>
          <td>Round Off:</td>
          <td style="text-align: right;">₹${bill.roundOff.toFixed(2)}</td>
        </tr>` : ''}
        <tr class="grand-total-row">
          <td>Grand Total:</td>
  ...
  <div class="footer-section">
    ...
    <div class="signature-box">
      <div style="font-size: 11px; color: #475569; font-weight: bold;">For ${company.tradeName || company.legalName}</div>
      <div class="signature-line">Authorized Buyer</div>
    </div>
  </div>
