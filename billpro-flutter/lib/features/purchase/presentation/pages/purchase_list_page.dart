...
class _PurchaseListPageState extends State<PurchaseListPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PurchaseProvider>().loadBills();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                onChanged: (v) => context.read<PurchaseProvider>().search(v),
                style: const TextStyle(fontSize: 14),
                decoration: const InputDecoration(
                  hintText: 'Search vendor, bill no...',
      ...
      body: p.isLoading
          ...
          : RefreshIndicator(
              onRefresh: () => context.read<PurchaseProvider>().loadBills(),
              child: p.bills.isEmpty
                  ? _EmptyState(
                  ...
                  : Column(children: [
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          _Stat(label: 'TOTAL BILLS', value: '${p.totalBills}', color: AppColors.textPrimary),
                          _divider(),
                          _Stat(label: 'DUE BILLS', value: '${p.dueBills}', color: const Color(0xFFD97706)),
                        ]),
                      ),
  ...
  Future<void> _openForm(BuildContext context, [PurchaseBillModel? bill]) async {
    final p = context.read<PurchaseProvider>();
    final messenger = ScaffoldMessenger.of(context);
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(builder: (_) => ChangeNotifierProvider.value(
        value: p,
        child: AddEditPurchasePage(bill: bill),
      )),
    );
    if (result == true && mounted) {
      AppToast.showOnMessenger(messenger,
          message: bill == null ? 'Bill created successfully!' : 'Bill updated successfully!',
          type: ToastType.success);
    }
  }

  Future<void> _confirmDelete(BuildContext context, PurchaseBillModel bill) async {
    final messenger = ScaffoldMessenger.of(context);
    final p = context.read<PurchaseProvider>();
    ...
    final confirmed = await showModalBottomSheet<bool>(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            'Bill ${bill.billNumber} will be permanently deleted. This action cannot be undone.',
    ...
    if (confirmed == true && mounted) {
      final success = await p.deleteBill(bill.id);
      if (success) {
        AppToast.showOnMessenger(messenger, message: 'Bill deleted', type: ToastType.success);
      } else {
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(bill.vendorName,
                  style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.textPrimary),
                  maxLines: 1, overflow: TextOverflow.ellipsis)),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(
                  color: bill.status == 'saved' ? AppColors.success.withValues(alpha: 0.1) : AppColors.primarySoft,
                  borderRadius: BorderRadius.circular(6),
                ),
                child: Text(bill.status.toUpperCase(),
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                        color: bill.status == 'saved' ? AppColors.success : AppColors.primary)),
              ),
            ]),
            const SizedBox(height: 4),
            Row(children: [
              Text(bill.billNumber, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary, fontFamily: 'monospace')),
              const Spacer(),
  ...
  Widget build(BuildContext context) => ListView(children: [
    SizedBox(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 20),
        Text(hasSearch ? 'No bills found' : 'No bills yet',
            style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        const SizedBox(height: 8),
        Text(hasSearch ? 'Try a different search term' : 'Start by creating your first purchase bill',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14, color: AppColors.textSecondary, height: 1.4)),
        if (!hasSearch) ...[
          const SizedBox(height: 24),
